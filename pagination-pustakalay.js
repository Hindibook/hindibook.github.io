/*<![CDATA[*/
var postperpage = 19;   // Posts per page
...
function loophalaman(banyakdata) {
  ...
  if (numshowpage % 2 === 0) {
    numshowpage = nomerkiri * 2 + 1; // Ensure numshowpage is odd
  }
  var mulai = nomerhal - nomerkiri;
  if (mulai < 1) mulai = 1;

  var maksimal = Math.floor((banyakdata + postperpage - 1) / postperpage); // Total pages correctly rounded up
...
function redirectpage(numberpage) {
  ...
  newInclude.src = home_page + "feeds/posts/summary?start-index=" + jsonstart + "&max-results=" + postperpage + "&alt=json-in-script&callback=finddatepost";
...
function redirectlabel(numberpage) {
  ...
  var newInclude = document.createElement('script');
  newInclude.type = 'text/javascript';
  newInclude.src = home_page + "feeds/posts/summary/-/" + lblname1 + "?start-index=" + jsonstart + "&max-results=" + postperpage + "&alt=json-in-script&callback=finddatepost";
  document.getElementsByTagName('head')[0].appendChild(newInclude);
}
...
function redirectsearch(numberpage) {
  ...
  var newInclude = document.createElement('script');
  newInclude.type = 'text/javascript';
  newInclude.src = home_page + "feeds/posts/summary?q=" + encodeURIComponent(searchQuery) + "&start-index=" + jsonstart + "&max-results=" + postperpage + "&alt=json-in-script&callback=finddatepost";
  document.getElementsByTagName('head')[0].appendChild(newInclude);
}
